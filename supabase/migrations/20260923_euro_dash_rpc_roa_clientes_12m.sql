-- ROA Invest por cliente (12m): receita demonstrativo / base média positivador.
-- Seguro e consórcio ficam de fora (não entram no demonstrativo).

CREATE OR REPLACE FUNCTION euro_dash.parse_br_numeric(value text)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    WHEN value ~ ',' THEN NULLIF(replace(replace(btrim(value), '.', ''), ',', '.'), '')::numeric
    ELSE NULLIF(btrim(value), '')::numeric
  END;
$$;

DROP FUNCTION IF EXISTS euro_dash.rpc_get_roa_clientes_12m(text[]);
...
AS $$
  ...
  WHERE (
      COALESCE(r.receita_invest_12m, 0) <> 0
      OR COALESCE(b.base_media_12m, 0) <> 0
    )
    AND (
      NOT EXISTS (SELECT 1 FROM assessor_filter)
      OR a.assessor IN (SELECT af.assessor FROM assessor_filter af)
    )
  ORDER BY roa_invest_pct NULLS LAST, a.cod_cliente;
$$;

GRANT EXECUTE ON FUNCTION euro_dash.rpc_get_roa_clientes_12m(text[])
  TO anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION euro_dash.parse_br_numeric(text)
  TO anon, authenticated, service_role;
