...
type ModeloServirRow = {
  periodo: string;
  cod_assessor: string;
  indice_modelo_servir: number;
};
...
function parsePeriodo(value: unknown): string | null {
  ...
  if (br) return `${br[3]}-${br[2].padStart(2, "0")}-01`;
  return null;
}
...
function mapRow(row: IncomingRow): ModeloServirRow | null {
  const periodo = parsePeriodo(pick(row, "periodo", "Ano Mês", "Ano Mes", "Mês", "Mes"));
  const codAssessor = normalizeAssessor(pick(row, "cod_assessor", "Cód. Assessor", "Cod. Assessor", "Cód Assessor", "Cod Assessor"));
  const indice = toNumber(pick(row, "indice_modelo_servir", "Índice Modelo de Servir", "Indice Modelo de Servir"));
  if (!periodo || !codAssessor || indice == null) return null;
  return {
    periodo,
    cod_assessor: codAssessor,
    indice_modelo_servir: indice,
  };
}

function dedupeRows(rows: ModeloServirRow[]) {
  const seen = new Map<string, ModeloServirRow>();
  for (const row of rows) seen.set(`${row.periodo}|${row.cod_assessor}`, row);
  return [...seen.values()];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = dedupeRows(incoming.map(mapRow).filter((row): row is ModeloServirRow => row != null));
    const ignoradas = incoming.length - rows.length;
    if (rows.length === 0) {
      return json(400, {
        error: "Nenhuma linha válida (Ano Mês, Cód. Assessor e Índice Modelo de Servir são obrigatórios)",
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const { error: insertError } = await supabase.from("dados_modelo_servir").insert(batch);
    ...
    console.error("ingest-modelo-servir", error);
