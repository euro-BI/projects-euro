...
type CambioRow = {
  ...
  dt_mn_cli: string | null;
  dt_me_cli: string | null;
};
...
function parseDate(value: unknown): string | null {
  ...
  if (/^total$/i.test(raw) || /^filtros/i.test(raw)) return null;
...
function toReceita(value: unknown) {
  if (value == null || value === "") return null;
  if (typeof value === "number" && Number.isFinite(value)) {
...
function mapRow(row: IncomingRow): CambioRow | null {
  const data = parseDate(pick(row, "data", "Data"));
  if (!data) return null;
  return {
    data,
    tp_pessoa: toStringValue(pick(row, "tp_pessoa", "TP_PESSOA", "Tp Pessoa")),
    assessor: normalizeAssessor(pick(row, "assessor", "Assessor", "CD_ASSESSOR")),
    receita_a_dividir: toReceita(pick(row, "receita_a_dividir", "Receita a dividir")),
    dt_mn_cli: parseDate(pick(row, "dt_mn_cli", "DT_MN_CLI")),
    dt_me_cli: parseDate(pick(row, "dt_me_cli", "DT_ME_CLI")),
  };
}

function uniqueKey(row: CambioRow) {
  return [row.data, row.assessor, row.tp_pessoa, row.receita_a_dividir, row.dt_mn_cli, row.dt_me_cli].join("|");
}

function dedupeRows(rows: CambioRow[]) {
  const seen = new Map<string, CambioRow>();
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = dedupeRows(incoming.map(mapRow).filter((row): row is CambioRow => row != null));
    const ignoradas = incoming.length - rows.length;
    if (rows.length === 0) {
      return json(400, { error: "Nenhuma linha válida (Data é obrigatória)", recebidas: incoming.length, ignoradas });
    }
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const { error: insertError } = await supabase.from("dados_cambio").insert(batch);
    ...
    console.error("ingest-cambio", error);
