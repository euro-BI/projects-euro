...
function mapRow(row: IncomingRow): RupturaRow | null {
  const periodo = parsePeriodo(pick(row, "periodo", "Ano Mês", "Ano Mes", "Mês", "Mes"));
  const codCliente = toStringValue(pick(row, "cod_cliente", "Cód. Conta", "Cod. Conta", "Cód Conta", "Cod Conta"));
  if (!periodo || !codCliente) return null;
  return {
    periodo,
    cod_assessor: normalizeAssessor(pick(row, "cod_assessor", "Cód. Assessor", "Cod. Assessor", "Cód Assessor", "Cod Assessor")),
    cod_cliente: codCliente,
    pontuacao: toInt(pick(row, "pontuacao", "Pontuação Ruptura", "Pontuacao Ruptura")),
  };
}

function uniqueKey(row: RupturaRow) {
  return `${row.periodo}|${row.cod_assessor}|${row.cod_cliente}`;
}

function dedupeRows(rows: RupturaRow[]) {
  const seen = new Map<string, RupturaRow>();
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = dedupeRows(incoming.map(mapRow).filter((row): row is RupturaRow => row != null));
    const ignoradas = incoming.length - rows.length;
    if (rows.length === 0) {
      return json(400, { error: "Nenhuma linha válida (Ano Mês e Cód. Conta são obrigatórios)", recebidas: incoming.length, ignoradas });
    }
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const { error: insertError } = await supabase.from("dados_rupturas").insert(batch);
    ...
    console.error("ingest-rupturas", error);
