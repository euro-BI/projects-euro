...
function mapRow(row: IncomingRow): CetipadoRow | null {
  ...
  return {
    data,
    assessor: normalizeAssessor(pick(row, "assessor", "Assessor")),
    cliente: toStringValue(pick(row, "cliente", "Cliente", "Conta")),
    fundo: toStringValue(pick(row, "fundo", "Fundo")),
    valor: toStringValue(pick(row, "valor", "Valor")),
    receita_estimada: toStringValue(pick(row, "receita_estimada", "Receita Estimada")),
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return json(400, {
        error: "Nenhuma linha válida (data é obrigatória)",
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const { error: insertError } = await supabase.from("dados_cetipados").insert(batch);
